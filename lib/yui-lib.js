        ...
        submodule = {
            updateCache: function(options, prop, submoduleName) {
                ...
                if (jsfiles) {
                    jsfiles.forEach(function(jsfile) {
                        ...
                        jsfile = libpath.join('js', jsfile).replace(/\\/g, '/');
            ...
            write: function(options, prop, submoduleName) {
                var opt = {},
                    prependFiles,
                    appendFiles,
                    path = buildProperties.getFilePath(options, submoduleName + '.js'),
                ...
                grunt.log.debug('Reading cache %s : %s', submoduleName, path);
            ...
            getFilesAsString: function(options, moduleName, files) {
                ...
                if (files) {
                    files.forEach(function(filename) {
                        var path = libpath.join(options.srcDir, moduleName, 'js', filename),
